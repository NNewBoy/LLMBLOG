...
def top_notes(
    ...
    for r in rows:
        ...
        if slug:
            slug_counts[slug] = int(r.c)
    result = []
    if slug_counts:
        notes = (
            db.query(Note)
            .filter(Note.slug.in_(list(slug_counts)), Note.deleted_at.is_(None))
            .all()
        )
        ...
        for slug, count in slug_counts.items():
            n = note_map.get(slug)
            if n:
                result.append({"id": n.id, "title": n.title, "slug": n.slug, "view_count": count})
    return ok(result)
